...
const SuggestionsHeader = () => {
  ...
  const { suggestions, sort, updateSort } = useGlobalContext();

  return (
    <div className="flex justify-center items-center suggestion_header">
      <header className="flex pl-6 items-center h-14 w-full bg-Raven-Night tablet:h-72 tablet:rounded-default tablet:w-689 desktop:w-825">
        <div className="flex h-full w-full justify-between items-center pr-3">
          <div className="flex gap-9">
            ...
            <form className="flex items-center justify-center">
              <label className="bg-Raven-Night text-white" htmlFor="sort">
                Sort by :
              </label>
              <select
                className="bg-Raven-Night text-white cursor-pointer relative"
                name="sort"
                id="sort"
                value={sort}
                onChange={updateSort}
              >
                {['Most Upvotes', 'Least Upvotes'].map((item, index) => {
                  return (
                    <option
                      key={index}
                      value={item}
